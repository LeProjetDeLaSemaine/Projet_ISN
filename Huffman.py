...
def construireArbre(dico):
	...
	tas = sorted(tas, reverse=True) #bof, mais confère une sûreté : si deux lettres ont le même nombre d'occurrences, le loop dans le dico échange parfois leur place (du fait de la nature non ordonnée des dictionnaires) -> le codage obtenu est donc toujours le même d'un test à l'autre
	while len(tas) >= 2:
		if debug: print("TAS = ", tas)
		tas = sorted(tas, key=lambda tup: tup[0], reverse=True) #on trie de façon décroissante (basé sur le 1er objet des tuples contenus dans la liste)
		n1, noeud1 = tas.pop() #on récupère le noeud de plus petit poids (la méthode pop renvoie et retire le dernier élément de la liste)
	...
	return tas[0][1] #à la fin, on a une liste contenant un unique tuple (d'où le [0]), qui contient un poids suivi d'un dictionnaire, que l'on veut (d'où le [1])
